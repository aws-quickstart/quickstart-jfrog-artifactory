...
== Updating {partner-product-short-name}
...
NOTE: If you plan to upgrade Artifactory and Xray, it is recommended to update Xray first.
...
. Modify the Xray version that is supported by the template, and then choose *Next* twice, choose the two *I acknowledge* check boxes, and choose *Update Stack*  (see <<xray_update_4>>).
...
. Shut down the Xray nodes one by one. The correct process is to shut down the nodes one at a time.  This will trigger a health check failure on the load balancer. The load balancer will then delete the current running node and deploy a net new primary node with the updated version.  (see <<xray_update_5>>).
...
will allow your Postgres database to be available from the internet.
